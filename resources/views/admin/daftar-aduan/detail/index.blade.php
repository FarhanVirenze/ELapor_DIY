...
@section('content')
    @php
        // AI Analysis Fallbacks (Unified with Superadmin)
        ...
        $sentiment = $report->effective_sentiment ?? $report->sentiment;
        if (!$sentiment) {
            $text = strtolower($report->judul . ' ' . $report->isi);
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <a href="{{ route('admin.kelola-aduan.index') }}" class="p-2 rounded-xl bg-gray-50 hover:bg-gray-100 text-gray-500 transition-all">
                            <i class="fas fa-arrow-left"></i>
                        </a>
                        <div>
                            <nav class="flex mb-1" aria-label="Breadcrumb">
                                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                                    <li class="inline-flex items-center text-[10px] font-bold uppercase tracking-widest text-gray-400">Admin OPD</li>
        ...
        <div x-data="{ openStatus: false, currentStatus: '{{ $report->status }}' }" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            ...
            <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 mb-8 relative overflow-hidden group hover:shadow-xl transition-all duration-500">
                ...
                @php
                    ...
                    foreach($steps as $idx => $step) {
                        if($step['key'] == $report->status) { $currentIndex = $idx; break; }
                    }
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden group hover:shadow-xl transition-all duration-300 flex-1 flex flex-col">
                        ...
                        <div class="p-6 md:p-8">
                            <div x-data="{ editing: false }">
                                ...
                                <form x-show="editing" action="{{ route('admin.reports.update', $report->id) }}" method="POST" x-cloak>
                                    @csrf @method('PUT')
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col" x-data="{ currentTab: 'tindak' }">
                        ...
                        <div class="p-6 md:p-8 min-h-[400px]">
                            <div x-show="currentTab == 'tindak'" x-transition class="flex flex-col h-full">
                                                ...
                                                <div class="bg-slate-50 rounded-2xl p-5 border border-slate-100 relative group">
                                                    <div class="flex items-center justify-between mb-3">
                                                        ...
                                                        <div class="flex items-center gap-2">
                                                            @if(($authUser->id_user === $item->user_id || $authUser->role === 'admin') && $item->user->role !== 'superadmin')
                                                                ...
                                                                <button onclick="openDeleteModal('{{ route('admin.reports.followup.delete', [$report->id, $item->id]) }}')" class="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-700 transition-all"><i class="fas fa-trash-alt"></i></button>
                                                            @endif
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div x-show="editing" x-cloak>
                                                        <form action="{{ route('admin.reports.followup.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                                            @csrf @method('PUT')
                                ...
                                <form action="{{ route('admin.reports.followup', ['id' => $report->id]) }}" method="POST" enctype="multipart/form-data" class="mt-8 pt-8 border-t border-slate-100 space-y-4 bg-white sticky bottom-0 z-10">
                                    @csrf
                            ...
                            <div x-show="currentTab == 'komentar'" x-transition class="flex flex-col h-full">
                                            ...
                                            <div class="group" x-data="{ editing: false, pesan: '{{ addslashes($item->pesan) }}' }">
                                                <div class="bg-white border border-slate-100 rounded-[2rem] p-6 shadow-sm relative transition-all group-hover:shadow-md">
                                                                ...
                                                                <div class="flex items-center gap-3">
                                                                    ...
                                                                    <div class="flex items-center gap-2">
                                                                        @if(($authUser->id_user === $item->user_id || $authUser->role === 'admin') && $item->user->role !== 'superadmin')
                                                                            ...
                                                                            <button onclick="openDeleteModal('{{ route('admin.reports.comment.delete', $item->id) }}')" class="opacity-0 group-hover:opacity-100 text-red-300 hover:text-red-700 transition-all"><i class="fas fa-trash-alt text-[10px]"></i></button>
                                                                        @endif
                                                                    </div>
                                                                </div>
                                                            </div>
                                                    ...
                                                    <div x-show="editing" x-cloak class="mt-2 pl-0 md:pl-14">
                                                        <form action="{{ route('admin.reports.comment.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                                                            @csrf @method('PUT')
                                ...
                                <form action="{{ route('admin.reports.comment', ['id' => $report->id]) }}" method="POST" enctype="multipart/form-data" class="mt-8 pt-8 border-t border-slate-100 flex items-center gap-4 bg-white sticky bottom-0 z-10">
                                    @csrf
                            ...
                            <div x-show="currentTab == 'lampiran'" x-transition class="space-y-6">
                                            ...
                                            <div class="relative group aspect-square rounded-2xl overflow-hidden border border-slate-200 bg-slate-50">
                                                 ...
                                                 <div class="absolute inset-0 bg-red-900/60 opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center p-4">
                                                     ...
                                                     <div class="flex gap-2">
                                                        ...
                                                        <form action="{{ route('admin.reports.file.delete', [$report->id, $index]) }}" method="POST">
                                                            @csrf @method('DELETE')
                                ...
                                <form action="{{ route('admin.reports.update', $report->id) }}" method="POST" enctype="multipart/form-data" class="mt-8 p-6 rounded-3xl bg-slate-50 border border-slate-100 flex flex-col sm:flex-row gap-4">
                                    @csrf @method('PUT')
                ...
                <div class="flex flex-col gap-8 sticky-column">
                    <!-- Informas Pelapor & Penanggung Jawab -->
                    <div class="glass-card rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/50 border border-white flex-1 flex flex-col">
                        ...
                        <div class="space-y-4 text-xs mb-8">
                            ...
                            <div class="flex items-center justify-between p-4 rounded-2xl bg-indigo-50/50 border border-indigo-100">
                                <span class="font-black text-indigo-400 uppercase tracking-wider">Interaksi</span>
                        ...
                        <!-- Section Penanggung Jawab (Moved here) -->
                        <div class="pt-6 border-t border-slate-100">
                            <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.1em] mb-4">Penanggung Jawab Tugas</h3>
                            <div class="bg-red-50/50 border border-red-100/50 rounded-2xl p-4 group relative overflow-hidden">
                                <div class="flex items-center gap-4">
                                    <div class="w-10 h-10 rounded-xl bg-red-100 text-red-600 flex items-center justify-center font-black shadow-sm">{{ substr($report->admin->name ?? 'A', 0, 1) }}</div>
                                    <div>
                                        <h4 class="text-xs font-black text-slate-800 leading-none">{{ $report->admin->name ?? 'Belum Ditugaskan' }}</h4>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm hidden z-[9999] flex items-center justify-center p-4">
        <div class="bg-white rounded-[2rem] p-8 w-full max-w-sm shadow-2xl text-center">
            ...
            <h3 class="text-xl font-bold text-slate-800 mb-2">Hapus?</h3>
            <p class="text-xs text-slate-400 mb-8">Tindakan ini tidak dapat dibatalkan.</p>
            <form id="deleteForm" method="POST" class="flex gap-4">
                @csrf @method('DELETE')
                <button type="button" onclick="closeDeleteModal()" class="flex-1 py-3 bg-slate-100 text-slate-500 font-bold rounded-xl">Batal</button>
                <button type="submit" class="flex-1 py-3 bg-red-600 text-white font-black rounded-xl">Hapus</button>
            </form>
        </div>
    </div>
@endsection
...
@section('include-js')
    ...
    <script>
        ...
        function openModal(src) { document.getElementById('modalImage').src = src; document.getElementById('imgModal').classList.remove('hidden'); }
